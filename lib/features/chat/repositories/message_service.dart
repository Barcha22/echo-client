...
class MessageService {
  ...
  Future<ApiResponse> sendMessage({
    ...
    if (recieverId.isEmpty) {
      return ApiResponse(
        status: 400,
        result: 'Receiver ID cannot be empty',
      );
    }
    if (content.isEmpty) {
      return ApiResponse(
  ...
  Future<ApiResponse> editMessage({
    ...
    if (newContent.isEmpty) {
      return ApiResponse(
        status: 400,
        result: 'New content cannot be empty',
      );
    }
    return await _api.post(
  ...
  Future<ApiResponse> getChatHistory(String friendId) async {
    if (friendId.isEmpty) {
      return ApiResponse(
        status: 400,
        result: 'Friend ID cannot be empty',
      );
    }
    return await _api.get(
      '${ApiConstants.getMessages}?friendId=$friendId',
    );
  }
